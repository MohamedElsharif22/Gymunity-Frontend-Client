...
import { HomeClientService, ProgramClientResponse, TrainerClientResponse } from '../../services/home-client.service';
import { forkJoin, of } from 'rxjs';
import { catchError } from 'rxjs/operators';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';

/**
 * Trainer Programs Component
 * Displays all programs from a specific trainer
 * Route: /discover/trainers/:id/programs
 */
@Component({
        ...
        @if (isLoading()) {
          <div class="bg-white rounded-lg shadow p-8">
            <div class="flex flex-col items-center justify-center">
              ...
              <p class="mt-4 text-gray-600">Loading trainer programs...</p>
            </div>
          </div>
        }
        ...
        @if (!isLoading() && !error()) {
          <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-sky-600 to-indigo-600 px-6 py-8 md:px-8">
              <h2 class="text-3xl font-bold text-white">Training Programs</h2>
              <p class="text-sky-100 mt-2">Explore all training programs from this trainer</p>
            </div>
            ...
            <div class="p-6 md:p-8">
              @if (programs().length === 0) {
                <div class="text-center py-16">
                  ...
                  <h3 class="text-2xl font-bold text-gray-900 mb-2">No Programs Found</h3>
                  <p class="text-gray-600">This trainer hasn't published any programs yet.</p>
                </div>
              } @else {
                    ...
                    <div class="bg-gradient-to-br from-gray-50 to-white border border-gray-200 rounded-lg hover:shadow-xl hover:border-sky-300 transition-all duration-300 overflow-hidden flex flex-col">
                      ...
                      <div class="p-6 flex-1 flex flex-col">
                        ...
                        @if (program.description) {
                          <p class="text-gray-600 text-sm mb-4 line-clamp-3 flex-1">{{ program.description }}</p>
                        }
                        ...
                        <div class="space-y-3 mb-6 pt-4 border-t border-gray-200">
                          <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-gray-900">{{ program.durationWeeks }} weeks</span>
                          </div>
                          <div class="flex items-center justify-between">
                            <span class="text-gray-600">Type</span>
                            <span class="font-semibold text-gray-900 capitalize">{{ program.type }}</span>
                          </div>
                          @if (program.price) {
                            <div class="flex items-center justify-between">
                              <span class="text-gray-600">Price</span>
                              <span class="text-xl font-bold text-sky-600">\${{ program.price }}</span>
                            </div>
                          }
                        </div>
                        ...
                        <div class="space-y-2 mt-auto">
                          ...
                          <button
                            (click)="enrollProgram(program.id)"
...
export class TrainerProgramsComponent implements OnInit {
  ...
  programs = signal<ProgramClientResponse[]>([]);
  ...
  loadData(): void {
    ...
    const trainerId = this.activatedRoute.snapshot.paramMap.get('id');
    
    if (trainerId) {
      this.fetchTrainerAndPrograms(trainerId);
    } else {
      this.error.set('No trainer ID provided');
    }
  }

  private fetchTrainerAndPrograms(trainerId: string): void {
    ...
    forkJoin({
      trainer: this.homeClientService.getTrainerById(parseInt(trainerId, 10)).pipe(
        catchError(err => {
          console.error('[TrainerProgramsComponent] Error loading trainer:', err);
          return of(null);
        })
      ),
      programs: this.homeClientService.getProgramsByTrainerProfile(parseInt(trainerId, 10)).pipe(
        catchError(err => {
          console.error('[TrainerProgramsComponent] Error loading programs:', err);
          return of([]);
        })
      )
    })
    ...
    .subscribe({
      next: (response: any) => {
        if (response.trainer) {
          ...
          console.log('[TrainerProgramsComponent] Trainer loaded:', response.trainer);
        }
        
        this.programs.set(response.programs || []);
        this.isLoading.set(false);
        console.log('[TrainerProgramsComponent] Programs loaded:', response.programs);
      },
      error: (err: any) => {
        this.isLoading.set(false);
        this.error.set('Failed to load trainer programs. Please try again.');
        console.error('[TrainerProgramsComponent] Error loading data:', err);
      }
    });
  }
  ...
  enrollProgram(programId: number): void {
    // Navigate to program details where enrollment can be done
    this.router.navigate(['/discover/programs', programId]);
  }
